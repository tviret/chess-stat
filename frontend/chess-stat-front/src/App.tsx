...
const App: React.FC = () => {
  ...
  const handlePlayerClick = useCallback(
    (nom: string) => push({ page: "player", nom }),
    [push],
  );

  const handleTournoiClick = useCallback(
    (id: number, nom: string) => push({ page: "tournament", id, nom }),
    [push],
  );

  const handleAdminClick = useCallback(() => push({ page: "admin" }), [push]);

  const handleSearch = useCallback(
    (query: string) => setFilters((f) => ({ ...f, joueur: query })),
    [],
  );
